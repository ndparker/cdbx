...
def clean(ctx, so=False, cache=False):
    ...
    path = ctx.get("terraform", {}).get("path")
    if path:
        ctx.shell.rm(
            ctx.shell.files(path, "*.auto.tfvars.json", recursive=False)
        )
        ctx.shell.rm(ctx.shell.files(path, "*.tf.json", recursive=False))
...
def cacheclean(ctx):
    ...
    path = ctx.get("terraform", {}).get("path")
    if path:
        ctx.shell.rm_rf("%s/.terraform" % (path,))
